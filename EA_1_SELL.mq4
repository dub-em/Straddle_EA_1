...
void uniformPointCalculator_sell(){
   ...
   nextTPSL = bestTp_sell(nextTPSL); //Adjusts the uniform TP incase the current one will result in loss.
...
double bestTp_sell(double currentTp){
   ...
   do{
      ...
      for(int i = OrdersTotal()-1; i >= 0; i--){
         OrderSelect(i, SELECT_BY_POS, MODE_TRADES);
         if(OrderMagicNumber() == magic_num){
            finalAmountAtClose += ((OrderOpenPrice() - currentTp)*10000) * (OrderLots()*10);
         }
      }
      add += 50;
